  ...
  function patch(oldVnode, vnode) {
    ...
    if (!oldVnode) {
      ...
      if (isRealElement) {
        ...
        var _el = vnode.el = oldVnode.el; // vnode 是新的虚拟节点


        updateProperties(vnode, oldVnode.data); // 4、比对儿子
    ...
    while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
      if (!oldStartVnode) {
        ...
        if (isSameVnode(oldStartVnode, newStartVnode)) {
          ...
          oldEndVnode = oldChildren[--oldEndIndex];
          newEndVnode = newChildren[--newEndIndex]; // 头移尾 (涉及到 倒叙 变成正序)
